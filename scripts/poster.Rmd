...
ppmath <- math %>%
  ...
  scale_x_continuous(breaks = c(-26.76 , 0 , 23.24),
                     ...
                     expand = c(0,0)) +
  labs(
    #title= "High school students proficienct in Math Test (2018-2019)",
       subtitle = "Student proficient in Reading varied between 28% to 90%.",
...
prla <- rla %>%
  ...
  labs(
    ...
    #title= "High school students proficienct in Reading Test (2018-2019)",
    subtitle = "Student proficient in Math varied between 29% to 94%",
    #captions = "Note: States missing from the graph did not report their data",
        x = "",
       y = "") +
    theme_minimal() +
...
ppmath + prla +
  plot_annotation(
    title = "High-chool proficient students in achievement tests"
  ) &
  plot_theme()
  
#ggpreview(width = 22, height = 15, units = "in")
...
geom_point(data=filter(state_r2b,
  ...
  theme_minimal()+
  xlim (14,108) +
    labs(title= "Textbook Spending & Math Achievement in High School",
         subtitle = "Textbook spending is moderately correlated with math achievement",
...
#ggpreview(height = 10, width = 12, unit = "in")
...
p1 <- ggplot(smw_change, aes(x = years, y = raw, group = re))+
    geom_point(aes(color = re), show.legend = FALSE, size = 3)+
...
p1 + p2 +
  ...
  subtitle = 'Covid-19 impacted student enrollment student enrollment across all groups\n ',
  caption = 'NHPI = Native Native Hawaiian or Other Pacific Islander \nAIAN = American Indian or Alaska Native \nNote: Missing data from Illinois'
  ) &
  plot_theme()

#ggpreview(width = 20, height = 12, unit = "in")
#ggsave(here::here("plot","change_v2.png"), width = 20, height = 12, unit = "in")
```
...
#ggpreview(width = 16, height = 10, unit = "in")
...
dotplot <- ggplot(st_binary, aes(x = percent_total, y = forcats::fct_reorder(st, percent_total))) +
    ...
    geom_point(aes(color = profession), size = 3)+
    geom_text(aes(x=percent_total,
                  y=st,
                  label = round(percent_total,0),
                  color = profession),
               nudge_x = 2, size = 7, family = "sans",
              show.legend = FALSE)+
...
#ggsave(here("plot","dotplot_v2.png"), height = 16, width = 12, unit = "in")
```
